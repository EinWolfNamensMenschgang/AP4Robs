...
const char* TRUTLEBOT_IP = "192.168.100.55";
...
//---------------------------------START WAYPOINT CREATION-------------------------------------
int waypointIndex = 0;
std::vector<Messages::Odometry_msg> waypoints;
waypoints.push_back(Functions::createWaypoint(0,0,0));
waypoints.push_back(Functions::createWaypoint(0.3,0,0));
waypoints.push_back(Functions::createWaypoint(0.3,0,-90));
waypoints.push_back(Functions::createWaypoint(0.5,-0.2,0));
waypoints.push_back(Functions::createWaypoint(0.7,0,90));
waypoints.push_back(Functions::createWaypoint(0.5,0.2,180));
waypoints.push_back(Functions::createWaypoint(0.3,0,270));
waypoints.push_back(Functions::createWaypoint(0.3,0,180));
waypoints.push_back(Functions::createWaypoint(0,0,180));
waypoints.push_back(Functions::createWaypoint(0,0,0));

//------------------------------END WAYPOINT CREATION------------------------------------
...
//-------------------------------START CONSUMER / TWIST_MSG PUBLISHER ---------------------------
// Laserscan and Odometry msgs from producer are received over shared memory.
// A Twist message is computed by the linear controller and published to the turtlebot
...
//------------TODO: Calculate Position From Laser Readings---------------
      double laserX;
      double laserY;
      double laserYAW;
//------------------------------------------------------------------------------
      ...
      double odomYAW = Functions::quaternionToYaw(nextConsumed.odom.orientation);
      ...
      double goalYaw = Functions::quaternionToYaw(goal.orientation);
      double k_alpha = 0.6;
      double k_rho = 0.3;
      double k_beta = -0.2;
      ...
      double delta_yaw = abs(Functions::rad2deg(goalYaw-assumedYAW));
      ...
      //std::cout << "Distance: " << lc_msg.distance << std::endl;
      std::cout << "DeltaYaw: " << delta_yaw << std::endl;
...
if(waypointIndex >= waypoints.size()){
   Publisher::publish(TRUTLEBOT_IP, "---START---{\"linear\": 0.0, \"angular\": 0.0}___END___");
}else if((lc_msg.distance < 0.03) && ((delta_yaw < 10)||(delta_yaw > 350))){ 
   waypointIndex++;
}else{
Publisher::publish(TRUTLEBOT_IP, twist_string);
}

      }  // end consuming loop
   }  // end consumer code

//---------------------------------END CONSUMER/TWIST_MSG Publisher-------------------------------

//---------------------------------START PRODUCER / ODOM & LASERSCAN SUBSCRIBER-----------------------------------
   ...
   { 
      ...
      while (true)
      {
        ...
        while(!castSuccess){
        std::thread OdomSub(Subscriber::subscribe, TRUTLEBOT_IP, ODOM_PORT, ODOM_RCVBUFSIZE, std::ref(odom_msg));
        std::thread LaserSub(Subscriber::subscribe, TRUTLEBOT_IP, SCAN_PORT, SCAN_RCVBUFSIZE, std::ref(laser_msg));
...
//-------------------------------END PRODUCER / ODOM & LASERSCAN SUBSCRIBER----------------------------------
}  // end main


//-------------------------------START ERROR HANDLING / DETACH SHARED MEMORY-------------------------------------
